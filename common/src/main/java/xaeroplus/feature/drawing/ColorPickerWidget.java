...
public class ColorPickerWidget extends AbstractWidget {
    ...
    private EditBox[] createColorEntryBoxes() {
        ...
        for (var channel : ColorChannel.values()) {
            ...
            entry.setResponder(value -> {
                if (!isValidEntryText(value)) {
                    var newValue = value.replaceAll("[^0-9]", "");
                    if (newValue.isBlank()) {
                        newValue = "0";
                    } else {
                ...
                updateColorFromEntry(channel, value);
            });
    ...
    @Override
    public void extractWidgetRenderState(final GuiGraphicsExtractor guiGraphics, final int mouseX, final int mouseY, final float partialTick) {
    ...
    private void renderColorPresets(final GuiGraphicsExtractor guiGraphics, final int mouseX, final int mouseY) {
        ...
        for (var i = 0; i < ColorHelper.HighlightColor.VALUES.length; i++) {
            ...
            if (selected || contains(mouseX, mouseY, x, y, presetWidth, PRESET_HEIGHT)) {
                renderOutline(guiGraphics, x, y, presetWidth, PRESET_HEIGHT, ColorHelper.getColor(255, 255, 255, 255));
            } else if (preset == ColorHelper.HighlightColor.BLACK) {
                renderOutline(guiGraphics, x, y, presetWidth, PRESET_HEIGHT, ColorHelper.getColor(128, 128, 128, 255));
            }
        }
    }

    private void renderColorEntries(final GuiGraphicsExtractor guiGraphics, final int mouseX, final int mouseY, final float partialTick) {
        ...
        for (var channel : ColorChannel.values()) {
            ...
            entry.extractWidgetRenderState(guiGraphics, mouseX, mouseY, partialTick);
        }
    }

    void renderColorGradient(final GuiGraphicsExtractor guiGraphics) {
    ...
    void renderHueBar(final GuiGraphicsExtractor guiGraphics) {
        var hueX = gradientX();
        var hueY = hueY();
    ...
    void renderAlphaBar(final GuiGraphicsExtractor guiGraphics) {
        var alphaX = alphaX();
    ...
    void renderSelectors(final GuiGraphicsExtractor guiGraphics) {
        ...
        guiGraphics.fill(
            ...
            hueY() - selectorExtension,
            hueSelectorX + 1,
            hueY() + hueBarHeight + selectorExtension,
            ColorHelper.getColor(0, 0, 0, 255)
        );
        ...
        guiGraphics.fill(
            alphaX() - selectorExtension,
            alphaSelectorY,
            alphaX() + alphaBarWidth + selectorExtension,
            alphaSelectorY + 1,
            ColorHelper.getColor(0, 0, 0, 255)
        );
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        return focusedEntry.keyPressed(event);
    }

    @Override
    public boolean charTyped(CharacterEvent event) {
        var focusedEntry = focusedEntry();
        return focusedEntry != null && focusedEntry.charTyped(event);
    }
