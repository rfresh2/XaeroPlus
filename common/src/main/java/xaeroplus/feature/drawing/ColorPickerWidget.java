...
public class ColorPickerWidget extends AbstractWidget {
    ...
    private EditBox[] createColorEntryBoxes() {
        ...
        for (var channel : ColorChannel.values()) {
            ...
            var entryX = x + consumedWidth + ENTRY_GAP * channel.ordinal();
    ...
    public void renderButton(final PoseStack guiGraphics, final int mouseX, final int mouseY, final float partialTick) {
        fill(guiGraphics, x, y, x + width, y + height, ColorHelper.getColor(0, 0, 0, 255));
    ...
    private void renderColorPresets(final PoseStack guiGraphics, final int mouseX, final int mouseY) {
    ...
    private void renderColorEntries(final PoseStack guiGraphics, final int mouseX, final int mouseY, final float partialTick) {
    ...
    void renderColorGradient(final PoseStack guiGraphics) {
    ...
    void renderHueBar(final PoseStack guiGraphics) {
        var hueX = gradientX();
        var hueY = hueY();
    ...
    void renderAlphaBar(final PoseStack guiGraphics) {
        var alphaX = alphaX();
    ...
    void renderSelectors(final PoseStack guiGraphics) {
    ...
    private void focusEntry(final EditBox selectedEntry) {
        for (int i = 0; i < colorEntryBoxes.length; i++) {
            ...
            if (entry == selectedEntry) {
                setEditBoxFocus(entry, true);
            } else {
                setEditBoxFocus(entry, false);
    ...
    private void setEditBoxFocus(EditBox editBox, boolean focus) {
        var current = editBox.isFocused();
        if (current == focus) return;
        editBox.changeFocus(focus);
    }
    ...
    int presetX(final int index) {
        ...
        return x + column * (baseWidth + PRESET_GAP) + Math.min(column, widerPresetCount);
    }

    int presetY() {
        return y + pickerHeight + PRESET_SECTION_GAP;
    }
