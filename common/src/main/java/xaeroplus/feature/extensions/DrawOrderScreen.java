...
public class DrawOrderScreen extends Screen {
    ...
    public void init() {
        ...
        addRenderableWidget(
            new Button(
                this.width / 2 - 100, this.height - 34,
                200, 20,
        ...
        addRenderableWidget(
            new Button(
                this.width - 82, 2,
                80, 20,
                Component.translatable("xaeroplus.gui.draw_order.reset"),
    ...
    public static class DrawFeatureList extends ObjectSelectionList<DrawFeatureEntry> {
        ...
        @Override
        public void setSelected(DrawFeatureEntry entry) {
            drawOrderScreen.selected = entry == null ? -1 : entry.index;
        ...
        public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
            ...
            renderBackdrop(poseStack);
            super.render(poseStack, mouseX, mouseY, partialTicks);
            if (dragging) {
                var draggedEntry = getEntry(dragged);
                draggedEntry.renderEntryText(poseStack, mouseX + draggedOffsetX, mouseY + draggedOffsetY);
        ...
        @Override
        protected void renderItem(PoseStack poseStack, int mouseX, int mouseY, float partialTick, int index, int left, int top, int width, int height) {
            if (!isSelectedItem(index)) {
                fill(poseStack,left - 2, top, left - 2 + width, top + height, ColorHelper.getColor(0, 0, 0, 150));
            }
            super.renderItem(poseStack, mouseX, mouseY, partialTick, index, left, top, width, height);
        }

        public void renderBackdrop(PoseStack poseStack) {
            fill(poseStack, 0, 0, width, height, ColorHelper.getColor(0, 0, 0, 100));
        }
    }
    ...
    public static class DrawFeatureEntry extends ObjectSelectionList.Entry<DrawFeatureEntry> {
        ...
        @Override
        public void render(final PoseStack poseStack, final int index, final int y, final int x, final int width, final int height, final int mouseX, final int mouseY, final boolean hovering, final float partialTick) {
            lastRenderX = x;
            lastRenderY = y;
            lastMouseX = mouseX;
            lastMouseY = mouseY;
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                if (drawFeatureList.getSelected() != this) {
                    drawFeatureList.setSelected(this);
                } else {
                    drawFeatureList.setSelected(null);
                }
