...
public abstract class AbstractEllipseDrawFeature<T> implements DrawFeature {
    ...
    public void preRender(final DrawContext ctx) {
        ...
        if (ctx.worldmap()) {
            ...
            XaeroPlusShaders.ELLIPSES_SHADER.setFrameSize(window.getWidth(), window.getHeight());
        }
        var thicknessScale = 16.0f * (float) Mth.clamp(
            thickness() * ctx.fboScale(),
            0.1f * (ctx.worldmap() ? 1.0f : Globals.minimapScaleMultiplier),
            1000.0f
        );
        XaeroPlusShaders.ELLIPSES_SHADER.setThickness(thicknessScale);
        XaeroPlusShaders.ELLIPSES_SHADER.setMapViewMatrix(ctx.untranslatedMapViewMatrix());
