...
public class MultiColorHighlightVertexBuffer extends AbstractHighlightVertexBuffer {
    ...
    public void refresh(DrawContext ctx, Long2LongMap highlights, int colorAlpha) {
        ...
        try (var byteBuffer = new ByteBufferBuilder(128)) {
            var bufferBuilder = new BufferBuilder(byteBuffer, PrimitiveTopology.QUADS, DefaultVertexFormat.POSITION_COLOR);
    ...
    public void render(DrawContext ctx, Long2LongMap highlights, int color) {
        ...
        try (var mappedView = dynamicTransformUniformBuffer.currentBuffer().map(false, true)) {
            var transform = new DynamicGpuData.Transform(RenderSystem.getModelViewStack(), new Vector4f(), new Vector3f(), new Matrix4f());
        ...
        try (final RenderPass pass = RenderSystem.getDevice().createCommandEncoder()
            ...
            pass.setPipeline(RenderSystem.getCompiledPipeline(XaeroPlusShaders.MULTI_COLOR_HIGHLIGHT_PIPELINE));
