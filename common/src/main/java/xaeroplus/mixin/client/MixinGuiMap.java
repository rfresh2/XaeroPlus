...
public abstract class MixinGuiMap extends ScreenBase implements IRightClickableElement {
    ...
    @Shadow
    private static ImprovedFramebuffer immediateRenderFBO;
    ...
    @Inject(method = "extractRenderState", at = @At(
        value = "INVOKE",
        target = "Lxaero/map/gui/GuiMap;init(II)V",
        ordinal = 1,
    ...
    @Redirect(method = "extractRenderState", at = @At(value = "FIELD", target = "Lxaero/map/gui/GuiMap;cameraX:D", opcode = Opcodes.PUTFIELD, ordinal = 1), remap = true)
    ...
    @Redirect(method = "extractRenderState", at = @At(value = "FIELD", target = "Lxaero/map/gui/GuiMap;cameraZ:D", opcode = Opcodes.PUTFIELD, ordinal = 1), remap = true)
    ...
    public RenderType injectCustomMapRenderType(final Operation<RenderType> original) {
        if (Settings.REGISTRY.transparentWorldmapBackgroundSetting.get()) {
            return XaeroPlusShaders.CUSTOM_MAP;
        }
    ...
    @WrapOperation(method = "extractRenderState", at = @At(
        value = "FIELD",
        opcode = Opcodes.GETSTATIC,
        target = "Lxaero/map/graphics/CustomRenderTypes;MAP_FRAME:Lnet/minecraft/client/renderer/rendertype/RenderType;"
    ))
    public RenderType injectCustomMapFrameRenderType(final Operation<RenderType> original) {
        if (Settings.REGISTRY.transparentWorldmapBackgroundSetting.get()) {
            return XaeroPlusShaders.CUSTOM_MAP_FRAME;
        }
        return original.call();
    }
    ...
    public void onImmediateFboRenderStateRegister(final CallbackInfo ci) {
        if (Settings.REGISTRY.transparentWorldmapBackgroundSetting.get()) {
            TextureUtils.clearRenderTarget(immediateRenderFBO, new Vector4f(0, 0, 0, 0), 0.0f);
        } else {
    ...
    public void renderDrawingStatusText(
        final CallbackInfo ci,
        @Local(argsOnly = true) final GuiGraphicsExtractor guiGraphics
    ) {
        if (!drawing) return;
    ...
    public void cancelClicksWhileDrawing(final MouseButtonEvent event, final boolean doubleClick, final CallbackInfoReturnable<Boolean> cir) {
        ...
        if (colorPickerActive && (event.button() == InputConstants.MOUSE_BUTTON_LEFT || event.button() == InputConstants.MOUSE_BUTTON_RIGHT)) {
        ...
        if (event.button() == InputConstants.MOUSE_BUTTON_LEFT) { // start drawing on left click
    ...
    public void drawingClickReleasedHandler(final MouseButtonEvent event, final CallbackInfoReturnable<Boolean> cir) {
        if (!drawing) return;
        if (colorPickerActive && (event.button() == InputConstants.MOUSE_BUTTON_LEFT || event.button() == InputConstants.MOUSE_BUTTON_RIGHT) && drawColorPicker.isMouseOver(event.x(), event.y())) {
            drawingLeftClickDown = false;
            drawingRightClickDown = false;
            drawInProgressPos = null;
        }
        ...
        if (event.button() == InputConstants.MOUSE_BUTTON_LEFT) { // stop drawing on left click release
            switch (drawingMode) {
                case LINE_SEGMENT, INFINITE_LINE -> {
                    if (drawInProgressPos != null) {
                        Line line;
            ...
            drawingLeftClickDown = false;
            cir.setReturnValue(true);
        } else if (event.button() == InputConstants.MOUSE_BUTTON_RIGHT) { // clear drawing on right click
            drawingRightClickDown = false;
            if (drawInProgressPos != null) return;
    ...
    public void panMouseButtonClick(final InputConstants.Type type, final int code, final CallbackInfoReturnable<Boolean> cir) {
        ...
        if (code != InputConstants.MOUSE_BUTTON_MIDDLE) return;
        if (!Settings.REGISTRY.worldMapUIAdditions.get()) return;
        pan = true;
    ...
    public void panMouseButtonRelease(final InputConstants.Type type, final int code, final CallbackInfoReturnable<Boolean> cir) {
        if (drawing) {
            if (type == InputConstants.Type.KEYBOARD && code == InputConstants.KEY_ESCAPE) {
                xaeroPlus$stopDrawing();
                cir.setReturnValue(true);
                return;
            }
            if (drawTextEntryActive) {
                if (type == InputConstants.Type.KEYBOARD && code == InputConstants.KEY_RETURN) {
    ...
    public void onInputPress(final KeyEvent event, final CallbackInfoReturnable<Boolean> cir) {
        ...
        if (Settings.REGISTRY.worldMapRotateHereKeybindSetting.getKeyBinding().matches(event)) {
            var mc = Minecraft.getInstance();
            mc.execute(() -> {
