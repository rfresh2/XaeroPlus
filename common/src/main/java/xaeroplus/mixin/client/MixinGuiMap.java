...
public abstract class MixinGuiMap extends ScreenBase implements IRightClickableElement {
    ...
    public void customInitGui(CallbackInfo ci) {
        ...
        zTextEntryField = new EditBox(Minecraft.getInstance().font, 20, xTextEntryField.y + 20, 75, 20, Component.nullToEmpty("Z:"));
        ...
        drawColorCyclerButton = new DrawingColorCyclerButton(
            startDrawingButton.x + 16, drawTextButton.y + 20,
        ...
        drawMeasurementToolButton = new GuiTexturedButton(
            startDrawingButton.x + 16, drawColorCyclerButton.y + 20, 20, 20, 135, 0, 16, 16,
            Globals.guiTextures,
        ...
        if (!SupportMods.pac()) {  // remove useless button when pac is not installed
            ...
            this.zoomOutButton.y = this.keybindingsButton.y - 20;
            this.zoomInButton.y = this.zoomOutButton.y - 20;
            this.switchToEndButton.y = this.zoomInButton.y - 20;
            this.switchToOverworldButton.y = this.switchToEndButton.y - 20;
            this.switchToNetherButton.y = this.switchToOverworldButton.y - 20;
        }
    ...
    public boolean hideRenderedStringsOnF1(final PoseStack matrixStack, final Font font, final String string, final int x, final int y, final int color, final float bgRed, final float bgGreen, final float bgBlue, final float bgAlpha, final VertexConsumer backgroundVertexBuffer) {
        return !Minecraft.getInstance().options.hideGui;
    }
    ...
    @WrapWithCondition(method = "render", at = @At(
        value = "INVOKE",
        target = "Lxaero/map/graphics/MapRenderHelper;drawCenteredStringWithBackground(Lcom/mojang/blaze3d/vertex/PoseStack;Lnet/minecraft/client/gui/Font;Lnet/minecraft/network/chat/Component;IIIFFFFLcom/mojang/blaze3d/vertex/VertexConsumer;)V"
    ), remap = true)
    public boolean hideMoreRenderedStringsOnF1(final PoseStack guiGraphics, final Font font, final Component text, final int x, final int y, final int color, final float bgRed, final float bgGreen, final float bgBlue, final float bgAlpha, final VertexConsumer backgroundVertexBuffer) {
        return !Minecraft.getInstance().options.hideGui;
    }
    ...
    @Unique
    private void xaeroPlus$setFocus(AbstractWidget b, boolean focus) {
    ...
    @WrapWithCondition(method = "render", at = @At(
        value = "INVOKE",
        target = "Lxaero/map/graphics/MapRenderHelper;fillIntoExistingBuffer(Lcom/mojang/math/Matrix4f;Lcom/mojang/blaze3d/vertex/VertexConsumer;IIIIFFFF)V",
        ordinal = 0
    ),
        slice = @Slice(
            from = @At(
    ...
    public void renderDrawingStatusText(
        final CallbackInfo ci,
        @Local(argsOnly = true) final PoseStack guiGraphics,
        @Local (name = "backgroundVertexBuffer") VertexConsumer backgroundVertexBuffer
    ) {
        if (!drawing) return;
