...
public abstract class MixinMinimapFBORenderer extends MinimapRenderer implements CustomMinimapFBORenderer {
    ...
    @Shadow
    private RenderSetup.TextureAndSampler scalingFramebufferNearestTAS;
    @Shadow
    ...
    public void reloadMapFrameBuffers() {
        if (!BuiltInHudModules.MINIMAP.getCurrentSession().getProcessor().canUseFrameBuffer()) {
            ...
            if (this.loadedFBO) {
                ...
                this.scalingFramebufferNearestTAS = new RenderSetup.TextureAndSampler(
                    this.scalingFramebuffer.getColorTextureView(), XaeroRenderType.getSimpleSampler(FilterMode.NEAREST)
                );
