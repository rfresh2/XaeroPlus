...
public abstract class MixinMinimapFBORenderer extends MinimapRenderer implements CustomMinimapFBORenderer {
    ...
    @Redirect(method = "renderChunksToFBO", at = @At(
        ...
        from = @At(
            value = "INVOKE",
            target = "Lxaero/common/graphics/ImprovedFramebuffer;getTas()Lnet/minecraft/client/renderer/rendertype/PreparedRenderType$Texture;"
        )
    ), remap = true)
    ...
    public void correctScaledFBO(final PoseStack matrixStack, final float x, final float y, final int textureX, final int textureY, final float width, final float height, final float textureH, final float factor, final RenderPipeline renderPipeline, final PreparedRenderType.Texture texture, final Operation<Void> original) {
        original.call(
            matrixStack,
