    ...
    private void getLinesFromBeaconBlockEntity(BeaconBlockEntity beaconBlockEntity, Object2IntMap<Line> linesCollector) {
        ...
        if (levels > 0) {
            ...
            if (!beamSections.isEmpty()) {
                var beaconColorArray = beamSections.get(beamSections.size()-1).getColor();
                lineColor = ColorHelper.getColor(Mth.ceil(beaconColorArray[0] * 255f), Mth.ceil(beaconColorArray[1] * 255f), Mth.ceil(beaconColorArray[2] * 255f), 255);
            }
            var d = levels * 10 + 10;
