...
public class XaeroPlusClientGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        try (var singleplayer = context.worldBuilder().create()) {
            ...
            singleplayer.getConnection().waitForChunksRender(20 * 600);
            ...
            waitFor(context, "drawing color picker", mc -> mc.gui.screen() != null && mc.gui.screen().children().stream()
            ...
            context.runOnClient(mc -> {
                mc.gui.setScreen(new GuiWorldMapSettings(mc.gui.screen(), null, BuiltInEditConfigScreenContexts.CLIENT));
            });
            takeScreenshot(context, "world_map_settings");

            context.runOnClient(mc -> {
                mc.gui.setScreen(new GuiMinimapMain(XaeroMinimapSession.getCurrentSession().getModMain(), mc.gui.screen(), null, true, BuiltInEditConfigScreenContexts.CLIENT));
            });
            takeScreenshot(context, "minimap_settings");

            context.runOnClient(mc -> {
                Settings.REGISTRY.waypointsListDistanceColumn.setValue(true);
                mc.gui.setScreen(new GuiWaypoints(HudMod.INSTANCE, BuiltInHudModules.MINIMAP.getCurrentSession(), null, null));
            });
        ...
        waitFor(context, "button containing " + expectedLabel, mc -> {
            ...
            for (var child : screen.children()) {
                if (child instanceof Button button && button.getMessage().getString().contains(expectedLabel)) {
                    button.onPress(new MouseButtonInfo(0, 0));
                    return true;
                }
            }
            return false;
        ...
        waitFor(context, "button " + buttonClass.getSimpleName(), mc -> {
            ...
            for (var child : screen.children()) {
                if (buttonClass.isInstance(child)) {
                    buttonClass.cast(child).onPress(new MouseButtonInfo(0, 0));
                    return true;
                }
            }
            return false;
