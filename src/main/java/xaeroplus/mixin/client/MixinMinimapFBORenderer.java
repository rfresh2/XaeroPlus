...
public abstract class MixinMinimapFBORenderer extends MinimapRenderer implements CustomMinimapFBORenderer {
    ...
    @Inject(method = "renderChunksToFBO", at = @At(
        value = "INVOKE",
        target = "Lnet/minecraft/client/gui/DrawContext;getMatrices()Lnet/minecraft/client/util/math/MatrixStack;"
    ), remap = true)
    public void modifyScaledSize(final XaeroMinimapSession minimapSession, final DrawContext guiGraphics, final MinimapProcessor minimap, final PlayerEntity player, final Entity renderEntity, final double playerX, final double playerZ, final double playerDimDiv, final double mapDimensionScale, final int bufferSize, final int viewW, final float sizeFix, final float partial, final int level, final boolean retryIfError, final boolean useWorldMap, final boolean lockedNorth, final int shape, final double ps, final double pc, final boolean cave, final boolean circle, final CustomVertexConsumers cvc, final CallbackInfo ci,
    ...
    public void removeDrawContextRef(final XaeroMinimapSession minimapSession, final DrawContext guiGraphics, final MinimapProcessor minimap, final PlayerEntity player, final Entity renderEntity, final double playerX, final double playerZ, final double playerDimDiv, final double mapDimensionScale, final int bufferSize, final int viewW, final float sizeFix, final float partial, final int level, final boolean retryIfError, final boolean useWorldMap, final boolean lockedNorth, final int shape, final double ps, final double pc, final boolean cave, final boolean circle, final CustomVertexConsumers cvc, final CallbackInfo ci) {
    ...
    public void modifyShaderMatrixStackTranslate(final MatrixStack instance, final float x, final float y, final float z,
                                                 @Share("scaledSize") LocalIntRef scaledSize) {
    ...
    public void modifyMMBackgroundFill(final DrawContext guiGraphics, final int x1, final int y1, final int x2, final int y2, final int color,
                                       @Share("scaledSize") LocalIntRef scaledSize) {
    ...
    public void drawRenderDistanceSquare(final XaeroMinimapSession minimapSession, final DrawContext guiGraphics, final MinimapProcessor minimap, final PlayerEntity player, final Entity renderEntity, final double playerX, final double playerZ, final double playerDimDiv, final double mapDimensionScale, final int bufferSize, final int viewW, final float sizeFix, final float partial, final int level, final boolean retryIfError, final boolean useWorldMap, final boolean lockedNorth, final int shape, final double ps, final double pc, final boolean cave, final boolean circle, final CustomVertexConsumers cvc, final CallbackInfo ci,
    ...
    @Redirect(method = "renderChunksToFBO", at = @At(
        value = "INVOKE",
        target = "Lxaero/common/minimap/render/MinimapRendererHelper;drawMyTexturedModalRect(Lnet/minecraft/client/util/math/MatrixStack;FFIIFFFF)V"
    ), remap = true)
    public void redirectModelViewDraw(final MinimapRendererHelper instance, final MatrixStack matrixStack, final float x, final float y, final int textureX, final int textureY, final float width, final float height, final float theight, final float factor,
                                      @Share("scaledSize") LocalIntRef scaledSize) {
