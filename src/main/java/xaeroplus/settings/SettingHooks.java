    ...
    public static synchronized void saveXPSettings() throws IOException {
        FileUtil.safeSave(XaeroPlus.configFile, w -> {
            try (PrintWriter writer = new PrintWriter(w)) {
                List<XaeroPlusSetting> allSettings = Settings.REGISTRY.getAllSettings().stream().sorted(Comparator.comparing(XaeroPlusSetting::getSettingName)).collect(Collectors.toList());
