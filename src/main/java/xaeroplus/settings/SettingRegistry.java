...
public abstract class SettingRegistry {
    private final Map<SettingLocation, ArrayList<XaeroPlusSetting>> settingLocationMap = new EnumMap<>(
        SettingLocation.class);
    private final Map<String, XaeroPlusSetting> settingNameMap = new ConcurrentHashMap<>();
    private final Map<KeyBinding, BooleanSetting> keybindingMap = new ConcurrentHashMap<>();
    ...
    private synchronized void register0(SettingLocation settingLocation, XaeroPlusSetting setting) {
        ...
        ArrayList<XaeroPlusSetting> settingList = this.settingLocationMap.getOrDefault(settingLocation, new ArrayList<>());
        settingList.add(setting);
        this.settingLocationMap.put(settingLocation, settingList);
        this.settingNameMap.put(setting.getSettingName(), setting);
        if (setting instanceof BooleanSetting) {
            KeyBinding kb = setting.getKeyBinding();
            if (kb != null) keybindingMap.put(kb, (BooleanSetting) setting);
        }
    }
    ...
    public BooleanSetting getKeybindingSetting(KeyBinding keyMapping) {
    ...
    public synchronized XaeroPlusSettingEntry[] getXaeroSettingEntries(SettingLocation settingLocation) {
        ArrayList<XaeroPlusSetting> settingList = this.settingLocationMap.get(settingLocation);
        if (settingList != null) {
            ...
            for (int i = 0; i < settingList.size(); i++) {
                ...
                XaeroPlusSettingEntry entry = xaeroPlusSetting.toXaeroSettingEntry();
